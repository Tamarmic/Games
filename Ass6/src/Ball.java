      ...
      if (gameEnvironment.getClosestCollision(way).collisionPoint() != null) {
        ...
        /*  if (a.getX()!=start.getX() || a.getY()!=start.getY()){
         ...
         // System.out.println(center.getX() + " " + center.getY() + "center after change place");
           setVelocity(gameEnvironment.getClosestCollision(way).collisionObject().
     ...
     if (this.center.getY() >= 570) {
        ...
        if (Math.abs(this.velocity.getDy()) < 1) {
         ...
         /*   double speed = Math.sqrt(Math.pow(velocity.getDx(), 2) + Math.pow(velocity.getDy(), 2));
            if (this.center.getX() >= this.paddle.getUpperLeft().getX()
                && this.center.getX() <= this.paddle.getUpperLeft().getX() + this.paddle.getWidth()
                && this.center.getY() >= this.paddle.getUpperLeft().getY()) {
                this.velocity = new Velocity(speed, 330);
                this.center = new Point(center.getX(),center.getY() -30);
                System.out.println("problemmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm");
            }*/
        }
